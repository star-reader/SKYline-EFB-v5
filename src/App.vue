<template>
  <div id="div" :class="isNightMode()">
    ...
    <div class="copyright">  <span>SKYline电子飞行包 | NavData API - <a target="__black" href="http://navigraph.com/">Navigraph</a> | <a target="__black" href="https://ww2.jeppesen.com/">Jeppesen</a></span> </div>
    <Nav></Nav>
    <Left></Left>
    <keep-alive>
...
watch:{
  ...
  methods: {
    ...
    loadUserInfo(halfWay){
      ...
      this.$axios.get(`https://api.skylineflyleague.cn/efb_api_v2/login?${new Date().getTime()}`,{headers:{'Authorization':`Bearer ${localStorage.getItem('access_token')}`}}).then((res)=>{
        if(!res.data.login){
           this.$message({
                    type: 'error',
                    message: '登录验证失败，请重新登录'
    ...
    showMessage(){
      if (localStorage.getItem('messageDate') != new Date().getDate()){
        setTimeout(()=>{
          ...
          this.$axios.get(`https://api.skylineflyleague.cn/efb_api_v2/bind?valid=${value}`,{headers:{
    ...
    setWidth(){
      ...
      try {
        if (that.width < 900) {
          document.getElementsByClassName('about-outer')[0].style.marginLeft = '0px'
        } else {
          document.getElementsByClassName('about-outer')[0].style.marginLeft = '60px'
        }
      } catch (error) {
      ...
      try {
        if (that.width < 900) {
          document.getElementsByClassName('per-outer')[0].style.marginLeft = '0px'
        } else {
          document.getElementsByClassName('per-outer')[0].style.marginLeft = '60px'
        }
      } catch (error) {
      ...
      try {
        if (that.width < 900) {
          document.getElementById("chart-content-weaper").style.marginLeft = "0px";
          document.getElementById("chart-content-weaper").style.width = "100%";
        } else {
          document.getElementById("chart-content-weaper").style.marginLeft = "60px";
          document.getElementById("chart-content-weaper").style.width = "calc(100% - 60px)";
        }
      } catch (error) {
      ...
      try {
        if (that.width < 900) {
          document.getElementById('ck-tab').style.marginLeft = '0px'
        } else {
          document.getElementById('ck-tab').style.marginLeft = '60px'
        }
      } catch (error) {
      ...
      try {
        if (that.width < 900) {
        document.getElementById("other-outer").style.marginLeft = "0px";
        document.getElementById("chart-display").style.width =
          "calc(100% - 400px)";
      } else {
        document.getElementById("other-outer").style.marginLeft = "60px";
        document.getElementById("chart-display").style.width =
          "calc(100% - 460px)";
      }
      } catch (error) {
      ...
      try {
        if (that.width < 900) {
        document.getElementById("sup-outer").style.marginLeft = "0px";
        document.getElementById("chart-display").style.width =
          "calc(100% - 400px)";
      } else {
        document.getElementById("sup-outer").style.marginLeft = "60px";
        document.getElementById("chart-display").style.width =
          "calc(100% - 460px)";
      }
      } catch (error) {
      ...
      try {
        ...
        document.getElementById('mobile-display-list').style.display = 'block'
        document.getElementById('left-icon').style.left = ''
        document.getElementById('left-icon').style.position = ''
      } else {
        document.getElementById('left').style.display = 'block';
        document.getElementById('mobile-display-list').style.display = 'none'
        document.getElementById('left-icon').style.left = '50px'
        document.getElementById('left-icon').style.position = 'relative'
      }
      } catch (error) {
